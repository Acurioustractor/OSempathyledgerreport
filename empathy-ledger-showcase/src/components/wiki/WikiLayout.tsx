...
import { wikiStructure, getAdjacentWikiPages } from '@/data/wiki-structure'
import { Book, Heart, Code, Users, FileText, Lightbulb, ChevronRight, ChevronLeft, Menu, X, Home } from 'lucide-react'
...
export default function WikiLayout({ children, currentSlug, pageTitle }: WikiLayoutProps) {
  const pathname = usePathname()
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false)
  const { previous, next } = getAdjacentWikiPages(currentSlug)
        ...
        <div className="flex items-center justify-between h-full px-4">
          <div className="flex items-center gap-4">
            ...
            <Link href="/wiki" className="flex items-center gap-2">
              ...
              <span className="font-semibold text-gray-900">Empathy Ledger Wiki</span>
            </Link>
          </div>
          <Link
            href="/"
                ...
                <div key={sectionIndex} className="mb-8">
                  ...
                  <ul className="space-y-1">
                    {section.pages.map((page, pageIndex) => {
                      const href = page.slug.startsWith('/') ? page.slug : `/wiki/${page.slug}`
